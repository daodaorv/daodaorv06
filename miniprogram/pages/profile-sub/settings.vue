...
const redirectUrl = ref('/pages/profile-sub/settings')
...
const sections = ref([
  {
    title: '账号信息',
    items: [
      { title: '头像与资料', note: '头像、昵称、生日', type: 'route', path: '/pages/profile-sub/edit' },
  ...
  },
  {
    title: '安全设置',
    items: [
      { title: '登录密码', note: '用于登录账号', type: 'route', path: '/pages/profile-sub/password-login' },
      { title: '支付密码', note: '支付及敏感操作验证', type: 'route', path: '/pages/profile-sub/password-pay' }
    ]
  },
  {
    title: '关于与支持',
...
const ensureAuth = (options: any) => {
  redirectUrl.value = buildRedirectUrl('/pages/profile-sub/settings', options || {})
  if (isLoggedIn()) {
    return true
  }

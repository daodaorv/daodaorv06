      ...
      <view class="itinerary-list">
        <view v-for="(day, index) in tourDetail.itinerary" :key="index" class="itinerary-item">
          <view class="day-header">
            <view class="day-number">D{{ Number(index) + 1 }}</view>
...
const {
  ...
  path: '/pages/business/tour/detail',
...
const bookTour = () => {
  ...
  // 跳转到预订页面
  uni.navigateTo({
    url: `/pages/business/tour/booking?tourId=${tourId.value}&batchId=${selectedBatch.value}`
  });
};
</script>

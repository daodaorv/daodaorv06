...
const handlePurchase = async () => {
	...
	if (!isLogin.value) {
		uni.navigateTo({
			url: `/pages/auth/login?redirect=/pages/business/membership/purchase?type=${purchaseType.value}`
		})
		return
	}
	...
	try {
		...
		const result = await mockPurchaseMembership(orderData as any)
	...
	try {
		...
		uni.showModal({
			...
			success: () => {
				// 跳转到会员中心
				uni.redirectTo({
					url: '/pages/business/membership/index'
				})
			}
		})
	} catch (error) {

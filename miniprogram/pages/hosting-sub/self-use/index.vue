      ...
      <view class="insurance-list">
        ...
        >
          <view class="insurance-select">
            <view class="radio" :class="{ checked: selectedInsurance === index }">
              <view v-if="selectedInsurance === index" class="radio-inner"></view>
            </view>
          </view>
...
export default {
  ...
  computed: {
    totalFee() {
      return this.fees.serviceFee + this.fees.insuranceFee + this.fees.returnFee
    },
    rentalDays() {
      if (!this.form.pickupDate || !this.form.returnDate) return 0
      const pickup = dayjs(this.form.pickupDate)
      const returnDay = dayjs(this.form.returnDate)
      return returnDay.diff(pickup, 'day')
    },
    pickupStoreDisplay() {
  ...
  methods: {
    ...
    calculateInsuranceFee() {
      ...
      if (days > 0) {
        this.fees.insuranceFee = this.insurancePlans[this.selectedInsurance].price * days
      } else {

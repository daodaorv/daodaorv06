...
#### 问题描述
用户反馈：点击"分享"按钮后，选择"分享给朋友"时，系统提示"请点击右上角分享"，而不是直接完成分享操作，用户体验不佳。

#### 根本原因
**微信小程序平台限制**：
- 微信小程序出于安全考虑，**不允许通过 JavaScript 代码直接触发分享对话框**
- 分享功能只能通过以下两种方式实现：
  1. 用户主动点击右上角菜单的"转发"按钮
  2. 使用 `<button open-type="share">` 组件

因此，原有的"分享给朋友"选项实际上无法实现直接分享，只能提示用户点击右上角，这不符合用户预期。

#### 修复方案
1. **移除"分享给朋友"选项** - 因为无法通过 JS 直接触发
2. **保留可用的分享方式**：
   - ✅ 生成海报（可分享到朋友圈）
   - ✅ 复制链接（手动分享）
   - ✅ 右上角分享（通过 `onShareAppMessage` 配置）

#### 修复内容
1. **useShare.ts** - 移除"分享给朋友"选项，调整索引映射
2. **ShareSheet.vue** - 移除"分享给朋友"选项
3. **coupon-mall/detail.vue** - 从旧的自定义实现改为使用 `useShare`

#### 修复效果
- ✅ 分享面板只展示可用的分享方式
- ✅ 用户可以通过右上角菜单直接分享小程序卡片
- ✅ 交互流程清晰，用户体验良好
...
#### 根本原因
通过控制台日志分析发现，uView Plus 的 `u-action-sheet` 组件的 `@select` 事件返回的是完整的 action 对象，而不是索引数字。原有的 `ShareSheet.vue` 组件尝试从对象中提取 `index` 或 `value` 属性，但这些属性并不存在，导致无法解析分享选项索引。

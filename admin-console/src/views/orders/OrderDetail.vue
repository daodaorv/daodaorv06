  ...
  <div class="order-detail-container">
    ...
    <el-card v-if="order && order.storeType === 'cooperative'" class="partner-card">
      ...
      <el-descriptions :column="3" border>
        ...
        <el-descriptions-item label="差价（分润基础）">
          <span v-if="order.priceDifference" style="color: #67c23a; font-weight: bold; font-size: 16px">
        ...
        <el-descriptions-item label="说明" :span="3">
          <div style="color: #909399; font-size: 13px; line-height: 1.6">
            合作商门店通过差价获利。差价 = 平台订单价格（¥{{ order.actualAmount.toLocaleString() }}）- 合作商订单价格（¥{{ order.partnerOrderPrice?.toLocaleString() || '0' }}）= ¥{{ order.priceDifference?.toLocaleString() || '0' }}
          </div>
        </el-descriptions-item>
      </el-descriptions>
    </el-card>
    ...
    <el-card v-if="order" class="actions-card">
      ...
      <div class="action-buttons">
        ...
        <el-button v-if="order.status === 'confirmed'" type="success" size="large" @click="handlePickup">取车登记</el-button>
        <el-button v-if="order.status === 'in_use'" type="warning" size="large" @click="handleReturn">还车登记</el-button>
    ...
    <!-- 编辑合作商订单信息对话框 -->
    <el-dialog v-model="partnerDialogVisible" title="编辑合作商订单信息" width="600px" @close="handlePartnerDialogClose">
      <el-form ref="partnerFormRef" :model="partnerForm" :rules="partnerFormRules" label-width="140px">
        ...
        <el-form-item label="差价（自动计算）">
          ...
          <div style="color: #909399; font-size: 12px; margin-top: 4px">
            差价 = 平台订单价格（¥{{ order?.actualAmount.toLocaleString() }}）- 合作商订单价格（¥{{ partnerForm.partnerOrderPrice?.toLocaleString() || '0' }}）
          </div>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="partnerDialogVisible = false">取消</el-button>
        <el-button type="primary" :loading="partnerLoading" @click="handlePartnerSubmit">确定</el-button>
      </template>
    </el-dialog>
...
const partnerFormRules: FormRules = {
  partnerOrderNo: [
    { required: true, message: '请输入合作商订单编号', trigger: 'blur' }
  ],
  partnerOrderPrice: [
...
const getOrderTypeTag = (type: string): 'primary' | 'success' | 'warning' | 'info' | 'danger' => {
  const map: Record<string, 'primary' | 'success' | 'warning' | 'info' | 'danger'> = { hosting: 'primary', cooperative: 'success', tour: 'warning' }
  return map[type] || 'info'
}
const getOrderTypeLabel = (type: string) => ({ hosting: '托管订单', cooperative: '合作订单', tour: '房车旅游' }[type] || type)

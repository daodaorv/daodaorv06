...
const mockIncomeRecords: IncomeRecord[] = Array.from({ length: 50 }, (_, i) => ({
  ...
  storeName: ['北京朝阳门店', '上海浦东门店', '深圳南山门店', '成都高新门店', '杭州西湖门店'][
...
const mockExpenseRecords: ExpenseRecord[] = Array.from({ length: 40 }, (_, i) => ({
  id: i + 1,
  category: ['vehicle', 'maintenance', 'insurance', 'salary', 'rent', 'marketing', 'other'][
...
const mockInvoices: Invoice[] = Array.from({ length: 30 }, (_, i) => ({
  ...
  status: ['pending', 'issued', 'sent', 'paid', 'cancelled'][
    Math.floor(Math.random() * 5)
  ] as InvoiceStatus,
 ...
 * 创建对账记录
 */
export function mockCreateReconciliation(
